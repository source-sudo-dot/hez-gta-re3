...
// two is the size that was settled on for the ring in play on the master branch
float CCrosshair::m_fSize = 2.0f;
...
// The shot that kills is registered as a hit first and comes through here after, so this
// only has the colour to add.  It must not take the shape away: the flag CDarkel is handed
// is only set when the game decides the kill was a head shot by its own rules, so a head
// shot often reaches here saying it was not one.  The hit knows better, it read the part
// off the collision itself.
void
...
// Draw2DPolygon walks its corners in the order 3, 4, 2, 1, so they are handed over in the
// order that makes that walk go round the shape.
static void
...
// Flat polygons come out with hard edges, and at the size this is set to the ring is only a
// couple of pixels across, so every stair step shows.  A slightly wider pass at part alpha
// underneath leaves a soft pixel along both edges, which is what the hardware would have put
// there had it been antialiasing the scene.
static void
...
{
	// Draw() runs once per rendered frame and the marker should fade in real time, so it is
	// counted down on the rendered frame length, which the time scale does not touch.
	if(m_fMarkerTime > 0.0f){
	...
	// a hairline of black either side of the ring, so the shape holds against a pale road as
	// well as a dark one
	...
	// A head shot puts a burst of spokes in the same place instead, so the two read apart by
	// their shape and not only by colour.
