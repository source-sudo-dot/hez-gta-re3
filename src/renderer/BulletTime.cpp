...
// Vice City has melee weapons besides the fists - knuckles, knives, bats - which have nothing
// to line up, so a gun is asked for rather than just anything that is not the fists.
bool
CBulletTime::IsAiming(void)
{
	CPlayerPed *player = FindPlayerPed();
	if(player == nil || FindPlayerVehicle() != nil || !CPad::GetPad(0)->GetTarget())
		return false;

	CWeaponInfo *info = CWeaponInfo::GetWeaponInfo(player->GetWeapon()->m_eWeaponType);
	return player->GetWeapon()->m_eWeaponType != WEAPONTYPE_UNARMED &&
...
CBulletTime::Process(void)
{
	...
	if(m_bActive){
		// a pill picked up off the street, or anything else that stops the adrenaline, takes
		// the meter with it
...
// A bar the width of the crosshair, a little under it.  Green while there is a full charge to
// spend, amber while it is running down, dim while it fills back up.
void
CBulletTime::DrawMeter(float x, float y)
{
	...
	// The bar used to be sized off the crosshair, which left it unreadable once the crosshair
	// was turned down small, so it keeps a size of its own.  Only where it sits still follows
	// the crosshair, far enough down to clear a large one.
