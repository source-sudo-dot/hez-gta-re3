...
Find3rdPersonCamTargetVectorFromCachedVectors(float dist, CVector pos, CVector& source, CVector& target, CVector camSource, CVector camFront, CVector camUp)
{
	if (CPad::GetPad(0)->GetLookBehindForPed()) {
		...
		// the same tangents the camera aims by, so the free aim shot lands on the crosshair too
		float tanX, tanY;
		TheCamera.Find3rdPersonCamAimTangents(TheCamera.Cams[TheCamera.ActiveCam].FOV, tanX, tanY);
		source = camSource;
		target = camFront;
		target += camUp * tanY;
